...
/**
 * test-all.mjs — Comprehensive test suite for jobhunt-india
 *
 ...
 * any freshly cloned jobhunt-india checkout with only Node.js available.
 *
 ...
 *
 * @param {string} path - Path relative to the jobhunt-india repository root.
...
const scripts = [
  ...
  // tests inside their active jobhunt-india workspace.
...
const systemFiles = [
  ...
  '.claude/skills/jobhunt-india/SKILL.md',
  '.opencode/skills/jobhunt-india/SKILL.md',
  '.antigravitycli/skills/jobhunt-india/SKILL.md',
];
...
// Add any private strings you never want committed (personal email, phone,
// home-directory path, etc.). Empty by default.
const leakPatterns = [
];
...
for (const skillPath of ['.claude/skills/jobhunt-india/SKILL.md', '.agents/skills/jobhunt-india/SKILL.md']) {
  if (!fileExists(skillPath)) {
    fail(`${skillPath} is missing`);
    continue;
  }
  const skill = readFile(skillPath);
  if (skill.includes('/jobhunt latex')) {
    pass(`${skillPath} exposes /jobhunt latex in discovery menu`);
  } else {
    fail(`${skillPath} does not expose /jobhunt latex in discovery menu`);
  }
}
...
try {
  const tmp = mkdtempSync(join(tmpdir(), 'jobhunt-india-portals-validator-'));
...
const canonicalSkill = '.agents/skills/jobhunt-india/SKILL.md';
const symlinks = [
  '.claude/skills/jobhunt-india/SKILL.md',
  '.opencode/skills/jobhunt-india/SKILL.md',
  '.antigravitycli/skills/jobhunt-india/SKILL.md',
...
{
  const fixtureRoot = mkdtempSync(join(tmpdir(), 'jobhunt-india-skills-'));
  try {
    const canonicalDir = join(fixtureRoot, '.agents', 'skills', 'jobhunt-india');
    const claudeDir = join(fixtureRoot, '.claude', 'skills', 'jobhunt-india');
    const opencodeDir = join(fixtureRoot, '.opencode', 'skills', 'jobhunt-india');
    mkdirSync(canonicalDir, { recursive: true });
    mkdirSync(claudeDir, { recursive: true });
    mkdirSync(opencodeDir, { recursive: true });

    const fixtureSkill = '---\nname: jobhunt-india\n---\n\n# canonical skill\n';
    const pointer = '../../../.agents/skills/jobhunt-india/SKILL.md';
...
{
  const fixtureRoot = mkdtempSync(join(tmpdir(), 'jobhunt-india-skills-unreadable-'));
  try {
    const canonicalDir = join(fixtureRoot, '.agents', 'skills', 'jobhunt-india');
    const claudeDir = join(fixtureRoot, '.claude', 'skills', 'jobhunt-india');
    mkdirSync(canonicalDir, { recursive: true });
...
{
  const fixtureRoot = mkdtempSync(join(tmpdir(), 'jobhunt-india-skills-entry-dir-'));
  try {
    const canonicalDir = join(fixtureRoot, '.agents', 'skills', 'jobhunt-india');
    const claudeDir = join(fixtureRoot, '.claude', 'skills', 'jobhunt-india');
    const opencodeDir = join(fixtureRoot, '.opencode', 'skills', 'jobhunt-india');
    ...
    const materialized = updater.materializeSkillEntrypoints(fixtureRoot);
    const opencodeSkill = readFileSync(join(opencodeDir, 'SKILL.md'), 'utf-8');
    if (JSON.stringify(materialized) === JSON.stringify(['.opencode/skills/jobhunt-india/SKILL.md']) && opencodeSkill === fixtureSkill) {
...
{
  const fixtureRoot = mkdtempSync(join(tmpdir(), 'jobhunt-india-skill-git-'));
  ...
  try {
    ...
    writeFileSync(join(claudeDir, 'SKILL.md'), pointer);
    writeFileSync(join(opencodeDir, 'SKILL.md'), pointer);
    gitRun(['add', '--', '.agents/skills/jobhunt-india/SKILL.md']);

    const pointerBlob = gitRun(['hash-object', '-w', '--stdin'], { input: pointer });
    gitRun(['update-index', '--add', '--cacheinfo', `120000,${pointerBlob},.claude/skills/jobhunt-india/SKILL.md`]);
    gitRun(['update-index', '--add', '--cacheinfo', `120000,${pointerBlob},.opencode/skills/jobhunt-india/SKILL.md`]);
    ...
    const claudeIndex = gitRun(['ls-files', '-s', '--', '.claude/skills/jobhunt-india/SKILL.md']);
    const opencodeIndex = gitRun(['ls-files', '-s', '--', '.opencode/skills/jobhunt-india/SKILL.md']);
    ...
    const claudeBlob = gitRaw(['show', ':.claude/skills/jobhunt-india/SKILL.md']);
    const opencodeBlob = gitRaw(['show', ':.opencode/skills/jobhunt-india/SKILL.md']);
...
try {
  ...
  const tmpDir = mkdtempSync(join(tmpdir(), 'jobhunt-india-migrate-'));
  try {
...
try {
  ...
  const dedupTmp = mkdtempSync(join(tmpdir(), 'jobhunt-india-dedup-'));
  try {
...
try {
  const rebuildTmp = mkdtempSync(join(tmpdir(), 'jobhunt-india-rebuild-'));
  try {
...
try {
  const mergeTmp = mkdtempSync(join(tmpdir(), 'jobhunt-india-merge-'));
  try {
...
try {
  const col912Tmp = mkdtempSync(join(tmpdir(), 'jobhunt-india-merge-912-'));
  try {
...
try {
  const mergeTmp = mkdtempSync(join(tmpdir(), 'jobhunt-india-merge-lock-'));
  /**
    ...
    const result = new Promise(resolve => {
      const child = spawn(NODE, ['merge-tracker.mjs'], {
        ...
        env: {
          ...
          CAREER_OPS_TRACKER_LOCK: join(mergeTmp, 'jobhunt-india-merge-tracker-fixture.lock'),
...
if (!sqliteAvailable) {
  ...
  try {
    const idxTmp = mkdtempSync(join(tmpdir(), 'jobhunt-india-index-'));
    try {
...
try {
  ...
  const hit = sj.detect({ name: 'SJ', careers_url: 'https://solid.jobs/public-api/offers/it?campaign=jobhunt-india' });
  ...
  const fakeJobs = {
    jobs: [
      { title: 'Senior Dev', url: 'https://solid.jobs/o/abc123/jobhunt', company: 'Acme Corp', locations: ['Warszawa', 'Remote'] },
      { title: 'Junior Dev', url: 'https://solid.jobs/o/def456/jobhunt', company: 'Beta Inc', locations: ['Kraków'] },
    ],
  };
  const parsed = await sj.fetch(
    { name: 'SolidJobs IT', careers_url: 'https://solid.jobs/public-api/offers/it?campaign=jobhunt-india' },
  ...
  if (parsed[0].title === 'Senior Dev' && parsed[0].url === 'https://solid.jobs/o/abc123/jobhunt') {
  ...
  const noCompanyJobs = { jobs: [{ title: 'Tester', url: 'https://solid.jobs/o/xyz/jobhunt', locations: [] }] };
  ...
  const mixedJobs = {
    jobs: [
      { title: 'Has URL', url: 'https://solid.jobs/o/1/jobhunt', company: 'A', locations: [] },
  ...
  const stringLocJobs = { jobs: [{ title: 'Dev', url: 'https://solid.jobs/o/2/jobhunt', company: 'X', locations: 'Warsaw' }] };
  ...
  const malformedMembers = { jobs: [null, 7, { title: 'OK', url: 'https://solid.jobs/o/3/jobhunt', company: 'Z' }] };
  const safeParsed = await sj.fetch(
    { name: 'SolidJobs IT', careers_url: 'https://solid.jobs/public-api/offers/it?campaign=jobhunt-india' },
    { transport: 'http', fetchJson: async () => malformedMembers, fetchText: async () => '' },
  );
  if (safeParsed.length === 1 && safeParsed[0].url === 'https://solid.jobs/o/3/jobhunt') {
...
try {
  ...
  // Release Please tags carry the component prefix (jobhunt-india-v1.9.0); the
  // prefix must be stripped or the releases-API fallback is dead code (#923).
  if (parse('jobhunt-india-v1.9.0') === '1.9.0') {
    pass('SEMVER_RE parses Release Please component-prefixed tag (jobhunt-india-v1.9.0 → 1.9.0)');
  } else {
    fail(`SEMVER_RE failed on jobhunt-india-v1.9.0 (got ${parse('jobhunt-india-v1.9.0')}) — releases-API fallback is dead code (#923)`);
  }
  ...
  if (parse('jobhunt-india') === null && parse('v1.9') === null) {
    pass('SEMVER_RE rejects non-semver input');
  } else {
    fail(`SEMVER_RE matched non-semver input (jobhunt-india → ${parse('jobhunt-india')}, v1.9 → ${parse('v1.9')})`);
  }
} catch (e) {
