#!/usr/bin/env node
/**
 * gemini-eval.mjs — Gemini-powered Job Offer Evaluator for jobhunt-india
 *
...
const PATHS = {
  ...
  evaluate:    join(ROOT, '.claude', 'skills', 'jobhunt-india', 'SKILL.md'),
  cv:          join(ROOT, 'cv.md'),
...
if (args.length === 0 || args[0] === '--help' || args[0] === '-h') {
  console.log(`
╔══════════════════════════════════════════════════════════════════╗
║           jobhunt-india — Gemini Evaluator (free-tier)             ║
╚══════════════════════════════════════════════════════════════════╝
...
function validateEvaluationShape(text) {
  ...
  if (issues.length > 0) {
    throw new Error(`Gemini returned an invalid jobhunt-india report: ${issues.join('; ')}`);
  }
}
...
// ---------------------------------------------------------------------------
const systemPrompt = `You are jobhunt-india, an AI-powered job search assistant.
...
// ---------------------------------------------------------------------------
console.log('\n' + '═'.repeat(66));
console.log('  JOB HUNTS AGENT INDIA — EVALUATION — powered by Google Gemini');
