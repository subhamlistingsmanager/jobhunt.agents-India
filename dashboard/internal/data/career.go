...
// relative to rootPath. Links are normally relative to the tracker
...
func resolveReportPath(rootPath, trackerPath, link string) string {
	resolved := filepath.Join(filepath.Dir(trackerPath), link)
	if _, err := os.Stat(resolved); err != nil {
		legacy := filepath.Join(rootPath, link)
		if _, err2 := os.Stat(legacy); err2 == nil {
			resolved = legacy
		}
	}
	if rel, err := filepath.Rel(rootPath, resolved); err == nil {
		return rel
	}
	return link
}
...
func ParseApplications(rootPath string) []model.CareerApplication {
	filePath := filepath.Join(rootPath, "applications.md")
	...
	for _, line := range lines {
		...
		// back to a rootPath-relative path, which is what every
		...
		if rm := reReportLink.FindStringSubmatch(fields[7]); rm != nil {
			...
			app.ReportPath = resolveReportPath(rootPath, filePath, rm[2])
		}
	...
	batchURLs := loadBatchInputURLs(rootPath)
	reportNumURLs := loadJobURLs(rootPath)

	for i := range apps {
		if apps[i].ReportPath == "" {
			continue
		}
		fullReport := filepath.Join(rootPath, apps[i].ReportPath)
	...
	enrichFromScanHistory(rootPath, apps)

	// Strategy 5: company name fallback from batch-input.tsv
	enrichAppURLsByCompany(rootPath, apps)

	return apps
}

// loadBatchInputURLs reads batch-input.tsv and returns a map of batch ID -> job URL.
func loadBatchInputURLs(rootPath string) map[string]string {
	inputPath := filepath.Join(rootPath, "batch", "batch-input.tsv")
...
func loadJobURLs(rootPath string) map[string]string {
	...
	statePath := filepath.Join(rootPath, "batch", "batch-state.tsv")
...
func enrichFromScanHistory(rootPath string, apps []model.CareerApplication) {
	scanPath := filepath.Join(rootPath, "scan-history.tsv")
...
func enrichAppURLsByCompany(rootPath string, apps []model.CareerApplication) {
	inputPath := filepath.Join(rootPath, "batch", "batch-input.tsv")
	inputData, err := os.ReadFile(inputPath)
	if err != nil {
		return
...
// Aliases match states.yml -- keep in sync with jobhunt-india states.yml
...
func LoadReportSummary(rootPath, reportPath string) (archetype, tldr, remote, comp string) {
	fullPath := filepath.Join(rootPath, reportPath)
...
func UpdateApplicationStatus(rootPath string, app model.CareerApplication, newStatus string) error {
	filePath := filepath.Join(rootPath, "applications.md")
	content, err := os.ReadFile(filePath)
	if err != nil {
		filePath = filepath.Join(rootPath, "data", "applications.md")
