...
func NewPipelineModel(t theme.Theme, apps []model.CareerApplication, metrics model.PipelineMetrics, rootPath string, width, height int) PipelineModel {
...
func (m PipelineModel) WithReloadedData(apps []model.CareerApplication, metrics model.PipelineMetrics) PipelineModel {
	...
	reloaded := NewPipelineModel(m.theme, apps, metrics, m.rootPath, m.width, m.height)
...
func (m PipelineModel) handleKey(msg tea.KeyMsg) (PipelineModel, tea.Cmd) {
	...
	case "enter":
		if app, ok := m.CurrentApp(); ok && app.ReportPath != "" {
			fullPath := filepath.Join(m.rootPath, app.ReportPath)
...
func (m PipelineModel) loadCurrentReport() tea.Cmd {
	...
	return func() tea.Msg {
		return PipelineLoadReportMsg{RootPath: path, ReportPath: report}
	}
}
...
func (m PipelineModel) renderHelp() string {
	...
	brand := lipgloss.NewStyle().Foreground(m.theme.Overlay).Render("Job Hunts Agent India")
