...
func (m *appModel) reloadPipelineData() {
	apps := data.ParseApplications(m.rootPath)
...
func (m appModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case screens.PipelineLoadReportMsg:
		archetype, tldr, remote, comp := data.LoadReportSummary(msg.RootPath, msg.ReportPath)
		m.pipeline.EnrichReport(msg.ReportPath, archetype, tldr, remote, comp)
		return m, nil

	case screens.PipelineUpdateStatusMsg:
		err := data.UpdateApplicationStatus(msg.RootPath, msg.App, msg.NewStatus)
		if err != nil {
...
func main() {
	pathFlag := flag.String("path", ".", "Path to Job Hunts Agent India directory")
	flag.Parse()

	rootPath := *pathFlag

	// Load applications
	apps := data.ParseApplications(rootPath)
	if apps == nil {
	...
	pm := screens.NewPipelineModel(t, apps, metrics, rootPath, 120, 40)

	for _, app := range apps {
		if app.ReportPath == "" {
			continue
		}
		archetype, tldr, remote, comp := data.LoadReportSummary(rootPath, app.ReportPath)
