 ...
 *
 * Run: node jobhunt-india/merge-tracker.mjs [--dry-run] [--verify]
 */
 ...
 * and use the jobhunt-india lock-name prefix. Invalid values are ignored and the
...
function resolveTrackerLockDir(envValue, lockKey) {
  ...
  const fallback = join(tmpRoot, `jobhunt-india-merge-tracker-${lockKey}.lock`);
  ...
  if (!basename(candidate).startsWith('jobhunt-india-merge-tracker-')) return fallback;
  return candidate;
}
