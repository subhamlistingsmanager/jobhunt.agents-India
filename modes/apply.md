...
Indian application forms (Naukri, Workday/Darwinbox portals, company forms) commonly ask: **Current CTC**, **Expected CTC**, **Notice Period**, **Current Location**, **Preferred Location**, and **willingness to relocate**. Treat all of these as `needs_candidate_confirmation: yes` unless the value is explicit in `config/profile.yml` (e.g. `cover_letter.notice_period_days`, comp bounds, location). Never guess Current or Expected CTC. Quote CTC in ₹ LPA. See `docs/INDIAN-JOB-SOURCES.md` for the comp glossary.
...
5. **jobhunt-india proof point**: Include in "Additional info" if there is a field for it
...
3. Suggest next step: `/jobhunt contacto` for LinkedIn outreach
