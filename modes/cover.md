...
Works in two modes:
- **Slug mode:** `/jobhunt cover {slug}` — loads the existing evaluation report draft as a starting point
- **Paste mode:** `/jobhunt cover` or JD pasted directly — starts from scratch
...
When the role is in India and the JD signals a start-date expectation (immediate/ASAP/joining timeline), state the candidate's **notice period / availability** plainly in the closing (e.g. "available to join after a 60-day notice"). Pull the value from `cover_letter.notice_period_days` in `config/profile.yml`; if it is absent, ask in Step 5 rather than guessing. Do not volunteer CTC in the letter — comp is negotiated separately and belongs in the application form, not the cover letter.
...
When invoked as `/jobhunt cover {slug}`:
