...
### Choosing the India template (do this first)

Pick the resume template based on `config/profile.yml`:

1. If `cv.template` is `"experienced"` → use `templates/cv-experienced-india.html`.
2. If `cv.template` is `"fresher"` → use `templates/cv-fresher-india.html`.
3. If `cv.template` is `"auto"` or absent → decide from `candidate_type`:
   - `candidate_type: experienced` → `cv-experienced-india.html`
   - `candidate_type: fresher` → `cv-fresher-india.html`
4. If neither field is set, infer: a CV with paid work experience → experienced template; a student/new-grad CV (no full-time roles, academics-led) → fresher template. When unsure, ask the user once.

**Defaults for India:** always render A4 (`{{PAGE_WIDTH}}` = `210mm`, `--format=a4`). No photo, DOB, marital status, or parent's name — those legacy fields hurt ATS parsing and are intentionally absent from both India templates.

**Design system:** both India templates share one token set (single navy accent, one type scale, one spacing rhythm) defined in `:root`. When tailoring or re-theming a résumé, follow `docs/RESUME-DESIGN.md` — change `--accent` in *both* templates to keep them a family, and never introduce a second accent colour.

- **Experienced template** (`cv-experienced-india.html`): Professional Summary → Core Skills → Work Experience → Key Achievements → Education → Certifications → optional Additional Details footer (Notice Period, Current/Expected CTC — include only if the user wants it on the resume; most omit it and share it in forms instead).
- **Fresher template** (`cv-fresher-india.html`): Career Objective → Education (10th %, 12th %, Degree+CGPA — first and prominent) → Technical Skills → Projects → Internships/Training → Certifications → Positions of Responsibility/Achievements → optional Coding Profiles (LeetCode/GitHub/HackerRank).

Both extend the same ATS-safe CSS as the original `cv-template.html`. The section order you render MUST match the section order in the user's `cv.md` (`generate-pdf.mjs` enforces this). See `templates/README.md` for the full placeholder list of each India template.

### Placeholders

Replace the `{{...}}` placeholders with personalized content:
...
- Or run `/jobhunt cover {slug}` later
```
