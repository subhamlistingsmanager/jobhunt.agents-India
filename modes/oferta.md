...
**India context — notice period & counter-offers:**
- **Notice-period strategy.** Indian roles commonly carry a 30/60/90-day notice. Decide the play early: serve the full notice, negotiate an early release, or **buy out** the balance (you or the new employer pays the shortfall). If the JD signals an immediate/ASAP start, surface this gap now and confirm the candidate's actual notice from `config/profile.yml`.
- **Counter-offer dynamics.** Resigning in India often triggers a counter-offer from the current employer (raise, retention bonus, role change). Note this as a likely event so the candidate decides their stance before resigning, rather than mid-negotiation. A counter-offer accepted under pressure rarely fixes the original reason for leaving — flag it, do not advise.

## Block D — Comp and Demand

Use WebSearch for:
- Current salaries for the role. **India sources:** AmbitionBox, Glassdoor India, Levels.fyi (apply the India filter), 6figr.com, Blind, LinkedIn Salary, PayScale India. Outside India, fall back to Glassdoor, Levels.fyi, Blind.
- Company's compensation reputation
- Demand trend for the role

**India comp framing (mandatory when the role is in India):**
- Discuss compensation as **total CTC in ₹ LPA** (lakhs per annum), not just headline salary.
- Break CTC into **fixed vs variable vs ESOP/RSU**. Variable realises below 100% in down years; ESOP value is notional until vesting + a liquidity event. Treat ESOP as upside, not cash.
- As a **rough orientation only (not a guarantee)**: monthly **in-hand ≈ 70–80% of fixed pay** after PF, professional tax, and income-tax (TDS) deductions. State this as an estimate; never present it as the candidate's guaranteed take-home.
- See `docs/INDIAN-JOB-SOURCES.md` for the comp & process glossary (CTC, in-hand, notice period, gratuity, EPF, etc.).
...
After saving the report and recording in the tracker, append a cover letter draft to the report file under `## Cover Letter Draft`. This is a starting point — not the final letter. The user completes it via `/jobhunt cover {slug}`.
...
> Draft generated at evaluation time. Complete via `/jobhunt cover {slug}` to fill in angles, confirm research, and generate the PDF.
...
---
*Run `/jobhunt cover {slug}` to complete angles, confirm company research, and generate the PDF.*
```
